...
def is_close(x1, x2):
  return tf.less(tf.abs(x1 - x2), eps)
...
def add_to_marg(params, alpha_c, x_c, y_c, beta_c, gamma_c,
 is_cur_c, all_vars):
  ...
  print_op = tf.cond(
    ...
    lambda: tf.Print(tf.constant(1.), [gamma_c], "ERROR: "),
...
def termn_condn(C_svm, all_vars):
  ...
  # with tf.control_dependencies([print_op]):
  return tf.logical_or(is_close(all_vars.g_c, 0.),
    is_close(all_vars.alpha_c, C_svm))
...
def handle_rem(params, min_rem_indx, gamma_rem, all_vars):
  ...
  all_vars = add_to_marg(params, alpha_, x_, y_,
    tf.constant(-1., dtype=tf.float64), tf.constant(-1., dtype=tf.float64),
    tf.constant(0.), all_vars)
...
def handle_marg(params, min_marg_indx, beta, all_vars):
  ...
  all_vars = tf.cond(tf.less(beta[min_marg_indx+1], 0),
    lambda: add_to_rem(params["threshold"], g_, x_, y_, all_vars),
...
def loop_body(params, i, all_vars):
  # printer = tf.Print(tf.constant(0), [all_vars.alpha_c,
  #   all_vars.g_c, all_vars.R, all_vars.Q_s], "Inside body", summarize=100)
...
def create_all_vars(params):
  ...
  all_vars = AllVars(
    ...
    marg_vec_x=tf.reshape(
      tf.get_variable("marg_vec_x", dtype=tf.float64).read_value(),
        [-1, x_shape]),
    marg_vec_y=tf.reshape(
      tf.get_variable("marg_vec_y", dtype=tf.float64).read_value(),
      [-1,]),
    alpha_marg=tf.reshape(
      tf.get_variable("alpha_marg", dtype=tf.float64).read_value(),
      [-1,]),
    err_vec_x=tf.reshape(
      tf.get_variable("err_vec_x", dtype=tf.float64).read_value(),
        [-1, x_shape]),
    err_vec_y=tf.reshape(
      tf.get_variable("err_vec_y", dtype=tf.float64).read_value(),
      [-1,]),
    g_err=tf.reshape(
      tf.get_variable("g_err", dtype=tf.float64).read_value(), [-1,]),
    rem_vec_x=tf.reshape(
      tf.get_variable("rem_vec_x", dtype=tf.float64).read_value(),
        [-1, x_shape]),
    rem_vec_y=tf.reshape(
      tf.get_variable("rem_vec_y", dtype=tf.float64).read_value(),[-1,]),
    g_rem=tf.reshape(
      tf.get_variable("g_rem", dtype=tf.float64).read_value(), [-1,]),
    x_c=tf.constant([], dtype=tf.float64), y_c=tf.constant(0., dtype=tf.float64),
    alpha_c=tf.constant(0., dtype=tf.float64),
...
def create_svm_variables(x_shape):
  with tf.variable_scope("svm_model") as scope:
    ...
    n_ = tf.get_variable("n", initializer=tf.constant(0), trainable=False,
      dtype=tf.int32)
    # offset
    b_ = tf.get_variable("b", initializer=tf.constant(0.,
    ...
    R_ = tf.get_variable("R",
      initializer=tf.constant([[INF]], dtype=tf.float64),
...
# Returns prediction
def svm_eval(x_test, model_params):
  ...
  with tf.Session() as sess:
    if tf.train.get_checkpoint_state('./svm_model/'):
      ...
      return sess.run(pred_y)
      # acc = sess.run(tf.reduce_mean(
      #   tf.cast(tf.equal(pred_y, labels), tf.int32)
      #   ))
      # if get_pred:
      #   return (acc, pred_y_out)
      # return acc
    else:
      # Raise error
...
def svm_train(x_train, y_train, model_params, restart=True, save_model=True):
  ...
  # type cast C
  model_params["C"] = tf.cast(model_params["C"], dtype=tf.float64)
